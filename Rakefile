...
namespace :generate do
  ...
  task :migration do
    ...
    migration_name      = ENV['NAME'].camelize
    migration_filename  = "%s_%s.rb" % [Time.now.strftime('%Y%m%d%H%M%S'), ENV['NAME'].underscore]
    migration_path      = APP_ROOT.join('db', 'migrate', migration_filename)

    if File.exist?(migration_path)
      raise "ERROR: Migration file '#{migration_path}' already exists"
    end
  ...
  task :spec do
    ...
    spec_name       = ENV['NAME'].camelize
    spec_filename   = "%s_spec.rb" % ENV['NAME'].underscore
    spec_path       = APP_ROOT.join('spec', spec_filename)
...
namespace :db do
  ...
  task :create do
    puts 'Creating development and test databases if they do not exist...'
  ...
  task :migrate do
    ...
    ActiveRecord::Migrator.verbose = ENV['VERBOSE'] ? ENV['VERBOSE'] == 'true' : true
...
# In a production environment like Heroku, RSpec might not be available.
# To handle this, rescue the LoadError.
begin
